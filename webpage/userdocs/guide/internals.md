...
Every open document lives in the ordered `DocumentStore` owned by the active window's
`ProjectContext`. Circuit, subcircuit, and code
...
Project state is never process-global: separate contexts can hold documents with the
same paths without affecting one another.

`CircuitDependencyGraph` is a runtime-only index rebuilt from circuit and subcircuit
