...
Circuit documents retain only their canonical path and serialized scene contents. Core
consumers parse the logical circuit section headlessly when needed; graphical scene
materialization remains a UI responsibility.

Subcircuit placeholders resolve their slug through the current `ProjectContext` and an
explicit `CircuitResolver`. Reusable-circuit interfaces use named `CircuitPort` values,
keeping each port name attached to its bus for elaboration and hierarchical Yosys export
rather than maintaining parallel name arrays. Elaboration operates on circuit documents
and never invokes Yosys.
