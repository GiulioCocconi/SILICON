...
portDefinitionsFromDocument(const nlohmann::json& document, bool inputPort)
{
  ...
  for (const auto& component : *componentsIt) {
    ...
    definitions.push_back(
        {.name    = std::move(name),
         .busSize = static_cast<unsigned int>(std::max<std::size_t>(1, wireIds.size())),
         .visualPosition = position,
         .wireIds        = std::move(wireIds)});
  }

  // Use visual order as the stable identity order for duplicate names and as fallback.
  std::ranges::sort(definitions, [](const PortDefinition& lhs,
                                    const PortDefinition& rhs) {
  ...
  // Boundary component names may collide; visible subcircuit ports still need unique names.
...
[[nodiscard]] QPoint defaultExternalPortPosition(
    const GraphicalSubcircuitMetadata& metadata, bool inputPort, std::size_t index,
    std::size_t count)
{
...
[[nodiscard]] std::vector<GraphicalSubcircuitPortMetadata> synchronizePorts(
    const std::vector<PortDefinition>& definitions,
    ...
    bool inputPort)
{
  ...
  for (std::size_t i = 0; i < definitions.size(); ++i) {
    ...
    synchronized.push_back({.name = definition.name,
                            .position = position,
                            .busSize = definition.busSize});
  }

  return synchronized;
}

[[nodiscard]] std::string fallbackPortName(const bool inputPort,
...
parseGraphicalSubcircuitMetadata(std::string_view sceneJson)
{
  try {
    ...
    metadata.widthHeight =
        QSize(gridToPixels(std::max(
                  1, shape.value("width", GraphicalSubcircuitDefaultSize))),
              gridToPixels(std::max(
                  1, shape.value("height", GraphicalSubcircuitDefaultSize))));
    ...
    std::string_view sceneJson, const GraphicalSubcircuitMetadata& fallbackMetadata)
{
  try {
    ...
    metadata.inputs =
        synchronizePorts(portDefinitionsFromDocument(document, true),
                         fallbackMetadata.inputs, metadata, true);
    metadata.outputs =
        synchronizePorts(portDefinitionsFromDocument(document, false),
        ...
        {.name     = hasFallback && !fallbackPorts[i].name.empty()
         ...
         .position = hasFallback ? fallbackPorts[i].position
                                 : defaultExternalPortPosition(metadata, inputPort, i,
                                                               buses.size()),
         .busSize  = static_cast<unsigned int>(std::max<std::size_t>(1, buses[i].size()))});
  }
