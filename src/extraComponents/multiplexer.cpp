...
void validateSelectionSize(const std::string_view componentName, const int selectionSize)
{
  if (selectionSize < 1)
...
{
  if (busSize <= 0)
    throw std::invalid_argument(std::string(componentName) + " busSize must be positive");
...
std::vector<Bus> resizedPorts(const std::vector<Bus>& currentPorts,
                              const std::size_t portCount, const unsigned short portWidth)
{
                                         ...
                                         const int selectionSize, const int busSize)
{
  ...
  } else if (currentInputs.size() > std::to_underlying(Multiplexer::Inputs::Selection)) {
  ...
  if (busSize == 1) {
    ...
    newInputs[std::to_underlying(Multiplexer::Inputs::Selection)] = std::move(selectionBus);
  ...
  const std::size_t oldDataBusCount = currentInputs.size() > 2 ? currentInputs.size() - 1 
...
void Multiplexer::simulate(Simulator& sim)
{
  ...
  if (busSize == 1) {
    ...
    const State selectedState =
        selectedIndex < dataSize ? inputState(Inputs::Data, static_cast<unsigned short>(selectedIndex)) : State::ERROR;
...
void Demultiplexer::simulate(Simulator& sim)
{
  ...
  if (busSize == 1) {
    ...
    for (unsigned short bit = 0; bit < outputBusSize(Outputs::Out); ++bit) {
      const State outputState = static_cast<std::size_t>(bit) == selectedIndex ? dataState : State::LOW;
  ...
  for (std::size_t output = 0; output < outputBuses().size(); ++output) {
    if (output == selectedIndex) {
      for (unsigned short bit = 0; bit < outputBuses()[output].size(); ++bit) {
        sim.updateWire(outputWire(output, bit), inputState(Inputs::Data, bit), delay, weak_from_this());
      }
    } else {
      for (unsigned short bit = 0; bit < outputBuses()[output].size(); ++bit) {
...
void Decoder::simulate(Simulator& sim)
{
  ...
  for (unsigned short bit = 0; bit < outputBusSize(Outputs::Out); ++bit) {
    const State outputState = (static_cast<std::size_t>(bit) == selectedIndex) ? State::HIGH : State::LOW;
    sim.updateWire(outputWire(Outputs::Out, bit), outputState, delay, weak_from_this());
  }
}
