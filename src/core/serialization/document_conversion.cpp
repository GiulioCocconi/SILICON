...
PreparedSemanticConversion prepareCircuitToVerilog(
    const Document& source, const SILICON::core::CircuitResolver& resolver)
{
  ...
  return {
      ...
      .execute =
          [path, circuit = std::move(circuit)](
            ...
            documents.push_back(
                {.path = path,
                 .payload = VerilogSource{.contents = SILICON::verilog::write(circuit)}});
...
PreparedSemanticConversion prepareVerilogToCircuits(
    const Document& source, const std::span<const Document> projectDocuments)
{
  ...
  for (const auto& document : projectDocuments) {
    if (document.getType() == DocumentType::Verilog)
      sources.push_back({.path = document.getPath(),
      ...
      .execute =
          [design = std::move(design)](
            ...
            for (const auto& module : exportOrder) {
              ...
              generated.push_back({
                  .path = SILICON::project::documentPathForSlug(
                      DocumentType::Circuit, module),
...
PreparedSemanticConversion prepareDocumentConversion(
    const Document& source, const DocumentType target,
    ...
    const SILICON::core::CircuitResolver& resolver)
{
  if (source.getType() == DocumentType::Circuit && target == DocumentType::Verilog)
    return prepareCircuitToVerilog(source, resolver);
