...
namespace {
  ...
  void validateInterface(const std::string_view          subcircuitKey,
                         ...
                         const std::vector<Bus>&         instanceBuses)
  {
    if (definitionPorts.size() != instanceBuses.size()) {
      throw std::runtime_error(std::format(
          ...
          subcircuitKey, direction, definitionPorts.size(), instanceBuses.size()));
    }
    ...
    for (std::size_t busIndex = 0; busIndex < definitionPorts.size(); ++busIndex) {
      ...
      throw std::runtime_error(std::format(
          ...
          subcircuitKey, direction, busIndex, definitionPorts[busIndex].bus.size(),
                                ...
                                std::vector<std::string>& activeSubcircuits)
  {
    ...
    WirePtrMap subcircuitWireMap;
    mapInterfaceWires(definition.inputs, externalInputs, subcircuitWireMap);
    mapInterfaceWires(definition.outputs, externalOutputs, subcircuitWireMap);

    appendCircuit(definition.circuit, context, std::move(subcircuitWireMap),
                  activeSubcircuits);
  }
