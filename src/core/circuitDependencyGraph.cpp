...
#include "circuitDependencyGraph.hpp"
...
void CircuitDependencyGraph::clear()
{
  graph.clear();
  pathToVertex.clear();
}

void CircuitDependencyGraph::addDocument(const std::string_view documentPath)
{
  const auto type = documentTypeForPath(documentPath);
  if (!type || !isCircuitDocument(*type))
    throw std::invalid_argument(
        "Circuit dependency graph entries must be Circuit documents");
...
void CircuitDependencyGraph::removeDocument(const std::string_view documentPath)
{
...
void CircuitDependencyGraph::validateDocumentRemoval(
...
void CircuitDependencyGraph::rebuildFromProject(const std::vector<Document>& documents)
{
  ...
  for (const auto& document : documents) {
    if (!isCircuitDocument(document.getType()))
      continue;
...
void CircuitDependencyGraph::replaceDocumentDependencies(
...
void CircuitDependencyGraph::validateDocumentDependencies(
...
bool CircuitDependencyGraph::wouldIntroduceCycle(const std::string_view documentPath,
...
CircuitDependencyGraph::DocumentPathList
CircuitDependencyGraph::dependentsOf(const std::string_view subcircuitPath) const
{
...
bool CircuitDependencyGraph::containsDocument(const std::string_view documentPath) const
{
  return findVertex(documentPath).has_value();
}

std::optional<CircuitDependencyGraph::Vertex>
CircuitDependencyGraph::findVertex(const std::string_view documentPath) const
{
...
CircuitDependencyGraph
CircuitDependencyGraph::withDocumentDependencies(const std::string_view documentPath,
                                                 const std::string_view contents) const
{
  auto updated = *this;
...
void CircuitDependencyGraph::replaceDependencyEdges(const std::string_view documentPath,
                                                    const std::string_view contents)
{
...
std::vector<std::string> CircuitDependencyGraph::findCycleTrace() const
{
...
std::vector<std::string> CircuitDependencyGraph::findCycleTraceFrom(
...
CircuitDependencyGraph::cycleTraceEndingAt(const Vertex               repeated,
...
CircuitDependencyGraph::Vertex
CircuitDependencyGraph::ensureVertex(const std::string_view documentPath)
{
...
void CircuitDependencyGraph::rebuildPathIndex()
{
  pathToVertex.clear();
...
void CircuitDependencyGraph::validateAcyclic() const
{
...
void CircuitDependencyGraph::throwIfCyclic() const
{
  try {
    validateAcyclic();
  } catch (const boost::not_a_dag&) {
