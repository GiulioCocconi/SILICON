...
TEST(DummyBusInputComponentTest, ResizesValuesWithoutIntegerConversion)
{
  auto input = std::make_shared<DummyBusInputComponent>(Bus(4), "input");

  input->setBusValue(busValueFromBits("1"));
  EXPECT_EQ(input->outputBuses()[0].getCurrentValue(), busValueFromBits("0001"));

  input->setProperty("size", 70);
  const BusValue wide(70, State::HIGH);
  input->setBusValue(wide);
  EXPECT_EQ(input->outputBuses()[0].getCurrentValue(), wide);
}

TEST(BusValueArithmeticTest, AddsKnownAndFourStateValues)
{
  EXPECT_EQ(busValueFromBits("0011") + busValueFromBits("0101"),
...
TEST(LogicTest, And)
{
  ...
  sim.setBus(Bus{a}, valueFor(Bus{a}, 0));  // LOW
  sim.setBus(Bus{b}, valueFor(Bus{b}, 0));  // LOW
  sim.run(20);
  EXPECT_EQ(o->getCurrentState(), State::LOW)
      << "AND(LOW, LOW) = " << to_str(o->getCurrentState());

  sim.setBus(Bus{a}, valueFor(Bus{a}, 1));  // HIGH
  sim.run(20);
  EXPECT_EQ(o->getCurrentState(), State::LOW)
      << "AND(HIGH, LOW) = " << to_str(o->getCurrentState());

  sim.setBus(Bus{b}, valueFor(Bus{b}, 1));  // HIGH
  sim.run(20);
...
TEST(SimulatorTest, CancellationStopsBusPropagation)
{
  ...
  EXPECT_EQ(simulator.setBus(Bus{a}, valueFor(Bus{a}, 1), []() { return true; }),
...
TEST(SimulatorTest, PassesSimulationContextToComponents)
{
  ...
  EXPECT_EQ(simulator.setBus(input, valueFor(input, 1)), Simulator::RunResult::Completed);
  ...
  EXPECT_EQ(simulator.setBus(input, valueFor(input, 0)), Simulator::RunResult::Completed);
...
TEST(SimulatorTest, SimulatesInternalInputWaveform)
{
  ...
  const std::vector<Sample> inputSnapshots{
      {0, {busValueFromBits("0"), busValueFromBits("0")}},
      {3, {busValueFromBits("1"), busValueFromBits("1")}},
  };
...
TEST(SimulatorTest, WaveformInputSampleCapturesPreviousWireStates)
{
  ...
  const std::vector<Sample> inputSnapshots{{0, {busValueFromBits("01")}}};
...
TEST(SimulatorTest, InteractiveSettlingKeepsWaveformChangesAtDistinctTimes)
{
  ...
  auto appendSnapshot = [&samples](uint64_t time, std::vector<BusValue> values) {
  ...
  simulator.setTraceSink([&](uint64_t time, const std::vector<BusValue>& values) {
  ...
  EXPECT_EQ(samples[1].values,
            (std::vector<BusValue>{busValueFromBits("1"), busValueFromBits("1")}));
  EXPECT_EQ(samples[2].values,
            (std::vector<BusValue>{busValueFromBits("0"), busValueFromBits("0")}));
}
...
TEST(SimulatorTest, WaveformAppliesSameTimestampInputsTogether)
{
  ...
  std::vector<BusValue> tracedValues;
  simulator.setTraceBuses({{"xor", Bus{o}}});
  simulator.setTraceSink([&](uint64_t, const std::vector<BusValue>& values) {
    if (!values.empty())
...
TEST(LogicTest, Nand)
{
  ...
  sim.setBus(Bus{b}, valueFor(Bus{b}, 0));
  sim.run(20);
  EXPECT_EQ(o->getCurrentState(), State::HIGH);
}
...
TEST(LogicTest, Or)
{
  ...
  sim.setBus(Bus{a}, valueFor(Bus{a}, 0));
  sim.setBus(Bus{b}, valueFor(Bus{b}, 0));
  sim.run(20);
  EXPECT_EQ(o->getCurrentState(), State::LOW)
...
TEST(LogicTest, CircuitEditing2)
{
  ...
  sim2.setBus(Bus{c}, valueFor(Bus{c}, 0));
  sim2.run(20);
  EXPECT_EQ(o->getCurrentState(), State::LOW);
}
...
TEST(LogicTest, BusSettingReading)
{
  ...
  for (int i = 0; i <= 0b1111; i++) {
    (void)a.forceSetCurrentValue(valueFor(a, static_cast<std::uint64_t>(i)));
    EXPECT_EQ(a.getCurrentValue(), valueFor(a, static_cast<std::uint64_t>(i)));
  }
}
...
TEST(LogicTest, BusSettingReadingAtUnsignedIntWidth)
{
  ...
  EXPECT_FALSE(bus.setCurrentValue(valueFor(bus, 0), {}));
  EXPECT_EQ(bus.getCurrentValue(), valueFor(bus, 0));
}

TEST(LogicTest, BusAssignmentsNormalizeWidthAndReportMeaningfulOverflow)
{
  Bus bus(4);

  EXPECT_FALSE(bus.forceSetCurrentValue(BusValue{State::HIGH}));
  EXPECT_EQ(bus.getCurrentValue(), busValueFromBits("0001"));

  EXPECT_FALSE(bus.forceSetCurrentValue(busValueFromBits("000001")));
  EXPECT_EQ(bus.getCurrentValue(), busValueFromBits("0001"));

  EXPECT_TRUE(bus.forceSetCurrentValue(busValueFromBits("100001")));
  EXPECT_EQ(bus.getCurrentValue(), busValueFromBits("0001"));

  EXPECT_TRUE(bus.forceSetCurrentValue(busValueFromBits("X00001")));
  EXPECT_EQ(bus.getCurrentValue(), busValueFromBits("0001"));
}
...
TEST(LogicTest, DisconnectedBusStateQueriesDoNotDereferenceNullWires)
{
  ...
  EXPECT_EQ(formatValue(bus.getCurrentValue(), BusValueFormat::Raw), "E");

  bus.setSize(2);
  EXPECT_TRUE(bus.isInErrorState());
  EXPECT_TRUE(bus.hasUnknowns());
  EXPECT_EQ(formatValue(bus.getCurrentValue(), BusValueFormat::Raw), "XE");
}
...
TEST(LogicTest, AndBitwiseSimulation)
{
  ...
  Simulator sim(circ);
  sim.setBus(gate->getInputs()[0], valueFor(gate->getInputs()[0], 0b1101));
  sim.setBus(gate->getInputs()[1], valueFor(gate->getInputs()[1], 0b1011));
  sim.run(20);
...
TEST(LogicTest, OrBitwiseSimulation)
{
  ...
  auto      circ = std::make_shared<Circuit>(Component_set{gate});
  Simulator sim(circ);
  sim.setBus(gate->getInputs()[0], valueFor(gate->getInputs()[0], 0b0101));
  sim.setBus(gate->getInputs()[1], valueFor(gate->getInputs()[1], 0b1010));
  sim.run(20);
...
TEST(LogicTest, NorBitwiseSimulation)
{
  ...
  Simulator sim(circ);
  sim.setBus(gate->getInputs()[0], valueFor(gate->getInputs()[0], 0b0101));
  sim.setBus(gate->getInputs()[1], valueFor(gate->getInputs()[1], 0b1000));
  sim.run(20);
